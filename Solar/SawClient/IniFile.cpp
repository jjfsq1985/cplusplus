...
namespace inifile
{
    ...
    int IniFile::getline(string &str, FILE *fp)
    {
        ...
        if (buf == NULL) {
            Tprintf(L"no enough memory!exit!\n");
            exit(-1);
        }
    ...
    int IniFile::load(const string &filename)
    {
        ...
        while (getline(line, fp) > 0)
        {
            ...
            else 
            {
                ...
                {
                    Tprintf(L"解析参数失败:Group %s Section %s: [%s]\n",strgroup, strsection, line.c_str());
                }
