...
if(@FType = 1)
	...
	where 1=1  and((FGunNo = @FGunno) or (@FGunno = 0)) and ((P.ClientName = @ClientName) or (@ClientName = '')) 
...
if (@FType = 3)
	begin
	select StationNo,GunNo,CardNo,TradeDateTime,GrayPrice,GrayGas,GrayMoney,ResidualAmount,Operator,OperateTime from Data_GreyCardRecord 
	where ((GunNo = @FGunno) or (@FGunno = 0)) and ((CardNo = @FCardNo) or (@FCardNo = ''))
	...
	left outer join (select FUserCardNo,SUM(FGas)FGas,SUM(FMoney)FMoney,MAX(FTradeDateTime)FTradeDateTime from SC_ConsumerDetail 
                   ...
                   group by FUserCardNo ) C On B.CardNum = C.FUserCardNo
