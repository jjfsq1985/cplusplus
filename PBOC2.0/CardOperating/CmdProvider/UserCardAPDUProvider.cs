...
{
    public class UserCardAPDUProvider : APDUBase
    {
        ...
        //安装主控密钥
        public bool createStorageKeyCmd(byte[] RandomVal)
        {
            ...
            //PriValue
            bufferData[8] = 0x01;
            Buffer.BlockCopy(m_KeyMain, 0, bufferData, 9, 16); //MF主控密钥作为数据
            //补齐32字节计算密文
            int nAppendLen = 7;
        ...
        public bool createWriteUserKeyCmd(byte[] randVal, StorageKeyParam Param)
        {
            ...
            byte[] encryptAsn = TripleEncryptData(Param.ASN, Param.StorageKey);
            byte[] encryptXorAsn = TripleEncryptData(Param.XorASN, Param.StorageKey);
            ...
            byte[] AllData = TripleEncryptData(encryptAll, Param.EncryptKey);
        ...
        public bool createSetStatusCmd(byte[] RandomVal, byte[] keyCalc)
        {
            ...
            if (keyCalc == null)
                ...
                key = keyCalc;
            byte[] cryptData = TripleEncryptData(RandomVal, key);
            m_Data = new byte[8];
