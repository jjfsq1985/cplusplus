...
{
    public class PSAMCardAPDUProvider : APDUBase
    {
        ...
        public bool createSetStatusCmd(byte[] RandomVal)
        {
        ...
        public bool createStorageAppKeyCmd(byte[] RandomVal, byte[] byteKey, byte Usage, byte Ver)
        {
            if (RandomVal.Length != 8 || byteKey.Length != 16)
                return false;
            m_CLA = 0x84;
            m_INS = 0xD4;
            m_P1 = 0x00;
            ...
            bufferData[2] = Ver;            
            bufferData[3] = 0x00;
            Buffer.BlockCopy(byteKey, 0, bufferData, 4, 16);
            //补齐24字节计算密文
            int nAppendLen = 4;
