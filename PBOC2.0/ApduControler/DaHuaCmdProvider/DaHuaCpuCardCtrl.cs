...
namespace DaHuaApduCtrl
{
    public class DaHuaCpuCardCtrl : DaHuaCardCtrlBase, IUserCardControl
    {
        ...
        private bool SelectFile(string strName, byte[] prefixData)
        {
            ...
            if (nRet < 0)
            {
                OnTextOutput(new MsgOutEvent(Color.Red.ToArgb(), "选择" + GetFileDescribe(strName) + "文件失败"));
                return false;
            }
            else
            {
        ...
        private bool CreateEF01()
        {
            ...
            if (nRet < 0)
            {
                OnTextOutput(new MsgOutEvent(Color.Red.ToArgb(), "创建目录EF01失败"));
                return false;
            }
            else
            {
        ...
        private bool StroageApplicationFile()
        {
            ...
            if (nRet < 0)
            {
                OnTextOutput(new MsgOutEvent(Color.Red.ToArgb(), "更新文件失败"));
                return false;
            }
            else
            {
        ...
        private bool CreateEFKeyFile()
        {
            ...
            if (nRet < 0)
            {
                OnTextOutput(new MsgOutEvent(Color.Red.ToArgb(), "创建气票交易密钥失败"));
                return false;
            }
            else
            {
        ...
        {            
            ...
            if (nRet < 0)
            {
                OnTextOutput(new MsgOutEvent(Color.Red.ToArgb(), "更新普通信息数据文件EF0B失败"));
                return false;
            }
            else
            {
        ...
        private bool UpdateEF10File(byte[] key)
        {
            ...
            if (nRet < 0)
            {
                OnTextOutput(new MsgOutEvent(Color.Red.ToArgb(), "更新灰锁文件失败"));
                return false;
            }
            else
            {
        ...
        {
            m_CmdProvider.createGetEFFileCmd(0x8B, 0x20);//基本数据(100+01011)0x0B文件长度32
            byte[] data = m_CmdProvider.GetOutputCmd();
            int datalen = data.Length;
            byte[] RecvData = new byte[128];
            int nRecvLen = 0;
